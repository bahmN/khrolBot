...
class PaymentController extends Controller {
    public function paymentLink($sum, $chatId, $period) {
        ...
        return $this->sendRequest('https://cryptoscan.one/api/v1/invoice/widget', json_encode($data), $headers)['data']['widget_url'];
    }
    ...
    public function responseCrypto(Request $request) {
        ...
        if ($webhookData['data']['status'] == 'paid') {
            ...
            $chat = Chat::where('chat_id', $chatId)->first();
            $chat->is_banned = 0;
            $chat->valid_until = $date->format('Y-m-d H:i:s');
            $chat->save();
